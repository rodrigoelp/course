...
filter p = foldRight (\a r -> (bool r ((:.) a r) (p a))) Nil
...
-- lengthGT4 ls = headOr False (find (<4) (foldRight (const (+1)) 1 ls))
...
notReverse = undefined -- no, is not possible.
-- notReverse ls = (filter even ls)
